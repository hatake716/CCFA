...
class ContainerManagerActivity : Activity() {
    ...
    private fun buildView(): View {
        ...
        content.addView(TextView(this).apply {
            this.text = "CCFAのLinux環境をアプリ内に複数保持"
            textSize = 14f
            setTextColor(muted)
            setPadding(0, dp(3), 0, dp(16))
        })

        val activeCard = section("アクティブコンテナ", "CCFAのターミナルが使用する環境")
    ...
    private fun renderRows(active: String?) {
        ...
        containers.forEachIndexed { index, name ->
            ...
            val box = LinearLayout(this).apply {
                ...
                background = rounded(
                    if (selected) Color.rgb(246, 232, 224) else soft,
    ...
    private fun dangerButton(value: String, click: () -> Unit) =
        styled(value, Color.rgb(245, 229, 226), danger, click)
